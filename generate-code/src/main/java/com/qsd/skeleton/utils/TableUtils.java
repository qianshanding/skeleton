...
import com.qsd.skeleton.model.TableBind;
import com.qsd.skeleton.model.TableCarray;
import com.qsd.skeleton.model.TableIndex;
...
public class TableUtils {
    ...
    public static final List<String> getTableNames(Connection conn)
        ...
        String tables = ConfigUtils.getConfig("tables");
        if ("*".equals(tables.trim())) {
            ...
            for (int i = 0; i < tablesArr.length; i++) {
                ResultSet rs = dme.getTables("", "", tablesArr[i], new String[]{"TABLE"});
                while (rs.next()) {
                    tableNames.add(rs.getString("TABLE_NAME"));
                }
            }
    ...
    public static final List<Table> getTables(Connection conn) throws Exception {
        List<Table> tables = new ArrayList<Table>();
        Table table = null;
        List<String> tabelNames = getTableNames(conn);
        ...
        for (String tableName : tabelNames) {
            ...
            String author = ConfigUtils.getConfig("author");//生成者
